/**
 * @module controllers/FileController.js
 */
...
class FilesController {
  ...
  static async postUpload(request, response) {
    ...
    if (fileParams.parentId !== 0 && !basicUtils.isValidId(fileParams.parentId)) { return response.status(400).json({ error: 'Parent not found' }); }
  ...
  static async getShow(request, response) {
    ...
    if (!basicUtils.isValidId(fileId) || !basicUtils.isValidId(userId)) { return response.status(404).json({ error: 'Not found' }); }
  ...
  static async getIndex(request, response) {
    ...
    if (parentId !== 0 && parentId !== '0') {
      if (!basicUtils.isValidId(parentId)) { return response.status(401).json({ error: 'Unauthorized' }); }
